...
class BoardRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAllBoards(userId: String?): List<Board> {
        // Se extrae la lista de .boards
        return api.getAllBoards(userId).body()?.boards?.map { it.toDomain() } ?: emptyList()
    }
    ...
    override suspend fun createBoard(name: String, description: String?, isPrivate: Boolean, isCollaborative: Boolean): Board {
        val response = api.createBoard(toCreateBoardRequest(name, description, isPrivate, isCollaborative))
        if (!response.isSuccessful) Log.e(TAG, "createBoard error ${response.code()}")
        return response.body()?.toDomain()
            ?: throw Exception("Error ${response.code()} al crear el tablero")
    }

    override suspend fun updateBoard(boardId: String, name: String, description: String?, isPrivate: Boolean, isCollaborative: Boolean, coverImageUrl: String?): Board {
    ...
    override suspend fun deleteBoard(boardId: String): Boolean = api.deleteBoard(boardId).isSuccessful
    ...
    override suspend fun removePinFromBoard(boardId: String, pinId: String): Boolean = api.removePinFromBoard(boardId, pinId).isSuccessful
    ...
    override suspend fun addCollaborator(boardId: String, userId: String, canEdit: Boolean, canAddPins: Boolean, canRemovePins: Boolean): BoardCollaborator {
    ...
    override suspend fun removeCollaborator(boardId: String, collaboratorUserId: String): Boolean = api.removeCollaborator(boardId, collaboratorUserId).isSuccessful

    override suspend fun updateCollaboratorPermissions(boardId: String, collaboratorUserId: String, canEdit: Boolean, canAddPins: Boolean, canRemovePins: Boolean): BoardCollaborator {
        val response = api.updateCollaboratorPermissions(boardId, collaboratorUserId, toUpdateCollaboratorRequest(canEdit, canAddPins, canRemovePins))
